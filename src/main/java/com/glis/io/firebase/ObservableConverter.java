 ...
 * @author Glis
 */
public abstract class ObservableConverter<T, V> {
    /**
     ...
     */
    public final Observable<Optional<T>> convert(final V object, final Class<T> clazz) throws Exception {
        final Subject<Optional<T>> subject = BehaviorSubject.create();
     ...
     */
    public abstract void link(V object, Subject<Optional<T>> subject, Class<T> clazz) throws Exception;
}
